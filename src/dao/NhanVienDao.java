...
public class NhanVienDao implements IDao<NhanVien, Integer> {
    ...
    public List<NhanVien> layHet() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                Date ngayDangKySql = rs.getDate("NgayDangKy");
                LocalDate ngayDangKy = ngayDangKySql != null ? ngayDangKySql.toLocalDate() : null;
                ...
                dsNhanVien.add(new NhanVien(maNhanVien, hoTen, gioiTinh, soDienThoai, ngayDangKy, ngaySinh, trangThai,
                        matKhau, email, vaiTro));
            }
        } catch (Exception e) {
    ...
    public boolean them(NhanVien entity) {
        try {
            String sql = "INSERT INTO nhanvien (HoTen, Gioitinh, SoDienThoai, NgaySinh, Trangthai, MatKhau, Email, VaiTro) "
                    + "VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    public NhanVien timNhanVienTheoSdt(String sdt) {
        try {
            ...
            while (rs.next()) {
                ...
                return new NhanVien(maNhanVien, hoTen, gioiTinh, soDienThoai, ngaySinh, trangThai, matKhau, email,
                        vaiTro);
            }
        } catch (Exception e) {
            Logger.getLogger(NhanVienDao.class.getName()).log(Level.SEVERE, null, e);
