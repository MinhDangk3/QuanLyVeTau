    ...
    public List<LichSuTraVe> layHet() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                Ve ve = new Ve(maVe, slot, khachHang, new Tuyen(maTuyen), new Tau(maTau), hoTenNguoiDi, cccdNguoiDi,
                        namSinhNguoiDi, tinhTrangVe);

                double phiTraVe = rs.getDouble("ls.PhiTraVe");
                String loaiTraVe = rs.getString("ls.LoaiTraVe");

                dsLichSuTraVe.add(new LichSuTraVe(maLichSuTraVe, thoiGianTraVe, ghiChu, phiTraVe, loaiTraVe, khachHang,
                        new NhanVien(maNhanVien), ve));
            }
        } catch (Exception e) {
    ...
    public boolean huyVe(List<TraVeModel> dsTraVe, KhachHang khachHang, NhanVien nhanVienThucHien) throws SQLException {
        ...
        try {
            ...
            if (slotDao.capNhatConSlot(con, dsCho)) {
                Logger.getLogger(LichSuTraVeDao.class.getName()).log(Level.INFO,
                        "Bước 2. Chuyển trạng thái thành còn chỗ");
            }

            PreparedStatement lichSuTraVeStmt = con.prepareStatement(
                    "INSERT INTO LichSuTraVe (MaKhachHang, MaVe, PhiTraVe, LoaiTraVe, MaNhanVienThucHien) " +
                            "VALUES (?, ?, ?, ?, ?)");
